...
map1 <- ggplot() + 
  ...
  geom_point(data = fqhc_census |> filter(Lat < 50 & Lat > 24) |> filter(year == 2019) ,
             aes(x = Long, y = Lat),
             alpha = .4,
             color = "darkgoldenrod1",
             size = 1) +
  labs(title    = "Map of FQHCs Registered in 2019") +
  coord_map() +
...
map2 <- ggplot() + 
  geom_polygon(data = urbnmapr::states, mapping = aes(x = long, y = lat, group = group),
               fill = "grey", color = "white") +
  geom_point(data = fqhc_census |> filter(Lat < 50 & Lat > 24) |> filter(year == 2011) ,
             aes(x = Long, y = Lat),
             alpha = .4,
             color = "darkgoldenrod1",
             size = 1) +
  labs(title    = "Map of FQHCs Registered in 2009") +
  coord_map() +
  theme(plot.title = element_text(size=5),
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
